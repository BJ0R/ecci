// resources/js/Pages/Teacher/Lessons/Index.jsx
...
export default function LessonsIndex({ lessons }) {
    ...
    function handleDelete() {
        router.delete(`/teacher/lessons/${deletingId}`, { onSuccess: () => setDeletingId(null) });
    }

    const ageGroupColors = { nursery: 'amber', kids: 'sky', youth: 'sage' };

    return (
        <TeacherLayout title="Lessons">
            <Head title="Lessons — ECCII Teacher" />
                ...
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {(!lessons.data || lessons.data.length === 0) && (
                            <tr>
                                <td colSpan={9} className="py-12 text-center text-[13px] text-stone-400">
                                    No lessons yet.{' '}
                                    <Link href="/teacher/lessons/create" className="no-underline font-medium text-amber-500">
                        ...
                        {lessons.data?.map(lesson => (
                            <tr key={lesson.id} className="transition-colors duration-100 hover:bg-amber-50">
                                <TableCell>
                                    <div
                                ...
                                <TableCell>
                                    <div className="font-semibold text-[13px] text-stone-900">{lesson.title}</div>
                                    {lesson.creator && (
                                        <div className="text-[10px] mt-0.5 text-stone-400">by {lesson.creator.name}</div>
                                    )}
                                </TableCell>
                                <TableCell muted>{lesson.series || '—'}</TableCell>
                                <TableCell mono>
                                    <span className="text-stone-400">Wk {lesson.week_number}</span>
                                </TableCell>
                                <TableCell>
                                    <Badge status={ageGroupColors[lesson.age_group] || 'amber'} size="xs">
                                        {lesson.age_group}
                                    </Badge>
                                </TableCell>
