// resources/js/Pages/Teacher/Lessons/Create.jsx
...
export default function LessonsCreate() {
    ...
    return (
        <TeacherLayout title="New Lesson">
            <Head title="Create Lesson — ECCII Teacher" />
                    ...
                    <div className="flex flex-col gap-5">
                        ...
                        <SectionCard title="Reflection Questions">
                            <p className="text-[12px] leading-relaxed mb-3.5 text-stone-400">
                                Add discussion questions for families to explore together.
                            </p>
                            {(data.reflection_questions || ['']).map((q, i) => (
                                <div key={i} className="flex items-start gap-2 mb-2.5">
                                    ...
                                    {(data.reflection_questions || []).length > 1 && (
                                        <button type="button" onClick={() => removeReflection(i)} className="flex-shrink-0 p-1.5 mt-1 rounded-lg border-none cursor-pointer hover:bg-rose-50 text-rose-500" style={{ background: 'none' }}>
                                            <X size={14} />
                                        </button>
                                    )}
                                </div>
                            ))}
                            <button type="button" onClick={addReflection} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-[12px] border-dashed border-2 border-stone-200 text-stone-400 transition-colors hover:border-amber-500 hover:text-amber-500" style={{ background: 'none', fontFamily: "'Outfit', sans-serif" }}>
                                <Plus size={13} />
                                Add Question
                            </button>
                        </SectionCard>
...
function SectionCard({ title, children }) {
    return (
        <div className="rounded-[14px] border border-stone-200 bg-white overflow-hidden">
            <div className="px-5 py-3.5 border-b border-stone-100 text-[12px] font-bold tracking-[0.02em] text-stone-900">{title}</div>
