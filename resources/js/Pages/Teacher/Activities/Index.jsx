// resources/js/Pages/Teacher/Activities/Index.jsx
...
export default function ActivitiesIndex({ activities }) {
    ...
    function handleDelete() {
        router.delete(`/teacher/activities/${deletingId}`, { onSuccess: () => setDeletingId(null) });
    }
    ...
    return (
        <TeacherLayout title="Activities">
            <Head title="Activities — ECCII Teacher" />
                ...
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {(!activities.data || activities.data.length === 0) && (
                            <tr>
                                <td colSpan={6} className="py-12 text-center text-[13px] text-stone-400">
                                    No activities yet.{' '}
                                    <Link href="/teacher/activities/create" className="no-underline font-medium text-amber-500">
                        ...
                        {activities.data?.map(activity => {
                            ...
                            return (
                                <tr key={activity.id} className="transition-colors duration-100 hover:bg-amber-50">
                                    <TableCell>
                                        <span className="font-semibold text-[13px] text-stone-900">{activity.title}</span>
                                    </TableCell>
                                    <TableCell>
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${tc.colorClass}`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                                            <tc.Icon size={11} />
                                            {tc.label}
                                        </span>
                                    </TableCell>
                                    <TableCell muted>
                                        {activity.lesson ? (
                                            <Link href={`/teacher/lessons/${activity.lesson.id}/edit`} className="text-[12px] no-underline font-medium hover:underline text-amber-500">
                                                {activity.lesson.title}
                                            </Link>
                                        ) : (
                                            <span className="text-[11px] italic text-stone-400">Standalone</span>
                                        )}
                                    </TableCell>
