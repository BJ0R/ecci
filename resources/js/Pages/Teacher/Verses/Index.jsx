// resources/js/Pages/Teacher/Verses/Index.jsx
...
export default function VersesIndex({ verses }) {
    ...
    function submit(e) {
        e.preventDefault();
        post('/teacher/verses', { onSuccess: () => { reset(); setShowForm(false); } });
    }

    function handleDelete() {
        router.delete(`/teacher/verses/${deletingId}`, { onSuccess: () => setDeletingId(null) });
    }

    return (
        <TeacherLayout title="Memory Verses">
            <Head title="Memory Verses — ECCII Teacher" />
            ...
            {showForm && (
                <div className="rounded-[14px] border border-stone-200 bg-white p-5 md:p-6 mb-6">
                    <div className="text-[20px] font-bold mb-5 text-stone-900" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            ...
            <div className="flex flex-col gap-3">
                ...
                {verses.data?.map(verse => (
                    <div key={verse.id} className="rounded-[14px] border border-stone-200 bg-white overflow-hidden flex flex-col sm:flex-row sm:items-stretch">
                        ...
                        <div className="flex-1 px-5 py-4 border-b border-stone-100 sm:border-b-0">
                            <div className="flex items-center gap-1.5 text-[9px] tracking-[0.18em] uppercase mb-2 text-amber-500" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                                <BookOpen size={10} />
                                Week {verse.week_number}
                            </div>
                            <blockquote className="text-[15px] italic leading-relaxed m-0 mb-2 text-stone-900" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                                "{verse.verse_text}"
                            </blockquote>
                            <div className="text-[11px] font-bold tracking-[0.06em] text-amber-500">— {verse.reference}</div>
                            {verse.context_note && (
                                <p className="text-[12px] leading-relaxed mt-2 m-0 text-stone-400">{verse.context_note}</p>
                            )}
                        </div>
                        <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-between px-4 sm:px-5 py-3 sm:py-4 sm:border-l border-stone-100 gap-3 sm:min-w-[140px]">
                            <div className="flex items-center gap-1.5">
                                ...
                                <Badge status="sage" size="sm">{verse.completions_count ?? 0} done</Badge>
                            </div>
                            <div className="text-[10px] text-stone-400" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
