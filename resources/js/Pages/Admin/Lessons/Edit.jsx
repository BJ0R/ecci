...
export default function LessonsEdit({ lesson }) {
    ...
    return (
        <AdminLayout title={`Edit: ${lesson.title}`}>
            <Head title={`Edit ${lesson.title} — ECCII Admin`} />
            ...
            <nav className="flex items-center gap-2 text-[12px] mb-6 text-stone-400">
                <Link
                    href="/admin/lessons"
                    className="inline-flex items-center gap-1 no-underline transition-opacity hover:opacity-70 text-stone-400"
                >
                    <ArrowLeft size={13} />
                    Lessons
                </Link>
                                ...
                                <div key={i} className="flex items-start gap-2 mb-2.5">
                                    ...
                                    {(data.reflection_questions || []).length > 1 && (
                                        <button
                                            ...
                                            className="flex-shrink-0 p-1.5 mt-1 rounded-lg cursor-pointer transition-colors hover:bg-rose-50 border-none text-rose-500"
...
function Card({ title, children }) {
    return (
        <div className="rounded-[14px] border border-stone-200 bg-white overflow-hidden">
            <div className="px-5 py-3.5 border-b border-stone-100 text-[12px] font-bold text-stone-900">
                {title}
            </div>
...
function InfoRow({ label, children }) {
    return (
        <div className="flex justify-between items-center py-1.5 border-b border-stone-100 last:border-b-0">
            ...
            <span className="text-[12px] text-stone-900" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                {children}
            </span>
        </div>
    );
